...
def pokaz_ekran_sprawdzania(okno, pokaz_glowny_ekran):
    ...
    # ======= Górny pasek =======
    top_bar = tk.Frame(okno, bg="#6A0DAD", height=50)  # fioletowe tło
    ...
    # ======= Główny content =======
    content = tk.Frame(okno, bg="#6A0DAD")  # fioletowe tło
    content.pack(expand=True, fill="both")

    label_literka = tk.Label(content, font=("Helvetica", 40, "bold"), bg="#6A0DAD")  # fioletowe tło
    ...
    if ProgramStates.sciezka_do_obrazka is not None:
        try:
            ...
            canvas = tk.Canvas(content, width=display_width, height=display_height, bg="#6A0DAD")  # fioletowe tło
            canvas.pack(pady=10)
            ...
            btn_frame = tk.Frame(content, bg="#6A0DAD")  # fioletowe tło
            ...
            tk.Label(content, text=f"Błąd ładowania obrazka: {e}", fg="red", bg="#6A0DAD").pack()
    else:
        label_literka.config(text="Brak wczytanej literki", image="")
        label_literka.image = None

    # ======= Label na wynik =======
    wynik_label = tk.Label(content, text="Wynik:", font=("Courier", 14), bg="#6A0DAD", fg="white", justify="left")
...
def pokaz_glowny_ekran(okno):
    ...
    # ======= STATUS MODELU =======
    ProgramStates.status_label = tk.Label(okno, text="⬜", font=("Helvetica", 16), fg="green", bg="#6A0DAD")
    ProgramStates.status_label.pack()

    # ======= NAGŁÓWEK =======
    naglowek = tk.Label(okno, text="Wybierz opcję:", font=("Helvetica", 16, "bold"), bg="#6A0DAD", fg="white")
